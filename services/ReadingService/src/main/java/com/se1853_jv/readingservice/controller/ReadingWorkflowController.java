...
public class ReadingWorkflowController {
    ...
    @Operation(summary = "Create a new reading workflow", 
               description = "Start reading a paper. Checks if workflow exists, creates new with status='unread', lastPage=0, progress=0")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get workflows by user", 
               description = "Get all reading workflows for a user, optionally filtered by status")
    @ApiResponses(value = {
    ...
    public ResponseEntity<WrapperApiResponse<List<ReadingWorkflowResponse>>> getWorkflowsByUser(
        ...
        List<ReadingWorkflowResponse> workflows = readingWorkflowService.getWorkflowsByUser(userId, status);
        return ResponseEntity.ok(WrapperApiResponse.success(workflows));
    }

    @PutMapping("/progress")
    @Operation(summary = "Update reading progress", 
               description = "Update lastPage and progress. If progress >= 100, status is auto-updated to 'finished'")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Update reading status", 
               description = "Update the status of a reading workflow")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Delete reading workflow", 
               description = "Delete a reading workflow and all associated notes/highlights mappings")
    @ApiResponses(value = {
