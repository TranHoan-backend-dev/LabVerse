...
public class UserApiHandler {
     ...
     * Lấy user bằng ID
     */
    public void getUserById(String id, ApiCallback<UserResponse> callback) {
        Log.d(TAG, "getUserById: " + id);
        Call<BaseJsonResponse<UserResponse>> call = userApi.getUserById(id);
        call.enqueue(new Callback<BaseJsonResponse<UserResponse>>() {
            @Override
            public void onResponse(@NonNull Call<BaseJsonResponse<UserResponse>> call, 
                                 @NonNull Response<BaseJsonResponse<UserResponse>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    var result = response.body().getData();
                    callback.onSuccess(result);
                    Log.d(TAG, "Get user by ID successful: " + result.getEmail());
                } else {
