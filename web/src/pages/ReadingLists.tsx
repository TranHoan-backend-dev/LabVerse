          ...
          {isLoading ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {readingLists.map((list: any) => (
                <Card key={list.id} className="shadow-custom-sm hover:shadow-custom-md transition-shadow">
                  <CardHeader>
                    <CardTitle className="flex items-start justify-between">
                      <span className="line-clamp-2">{list.name}</span>
                      <BookMarked className="h-5 w-5 text-muted-foreground flex-shrink-0 ml-2" />
                    </CardTitle>
                    {list.description && (
